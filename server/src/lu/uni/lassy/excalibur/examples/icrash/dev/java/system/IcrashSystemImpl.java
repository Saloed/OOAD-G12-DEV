...
import lu.uni.lassy.excalibur.examples.icrash.dev.java.system.db.*;
...
public class IcrashSystemImpl extends UnicastRemoteObject implements
    ...
    private List<CtAlert> getAlertsByCrisis(CtCrisis aCtCrisis) {
        ...
        return assCtAlertCtCrisis.keySet().stream().filter(ctAlert -> assCtAlertCtCrisis.get(ctAlert).id.value.getValue().equals(
                aCtCrisis.id.value.getValue())).collect(Collectors.toList());
    }
    ...
    public List<ActAdministrator> getAllActAdministrators()
            throws RemoteException {
        return assCtAuthenticatedActAuthenticated
                .keySet().stream().filter(ctAuth -> ctAuth instanceof CtAdministrator).map(ctAuth -> (ActAdministratorImpl) assCtAuthenticatedActAuthenticated
                        .get(ctAuth)).collect(Collectors.toList());
    }
    ...
    public ArrayList<CtAdministrator> getAllCtAdministrators() throws RemoteException {
        return cmpSystemCtAuthenticated
                .values().stream().filter(ctAuth -> ctAuth instanceof CtAdministrator).map(ctAuth -> (CtAdministrator) ctAuth).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public ArrayList<CtCoordinator> getAllCtCoordinators() throws RemoteException {
        return cmpSystemCtAuthenticated.values().stream().filter(auth -> auth instanceof CtCoordinator).map(auth -> (CtCoordinator) auth).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public ArrayList<CtAlert> getAllCtAlerts() throws RemoteException {
        ArrayList<CtAlert> result = new ArrayList<>();
        if (cmpSystemCtAlert != null) {
            result.addAll(cmpSystemCtAlert.values());
        }
        return result;
    }
    ...
    public ArrayList<CtHuman> getAllCtHumans() throws java.rmi.RemoteException {
        ArrayList<CtHuman> result = new ArrayList<>();
        if (cmpSystemCtHuman != null) {
            result.addAll(cmpSystemCtHuman.values());
        }
        return result;
    }
    ...
    public ArrayList<CtCrisis> getAllCtCrises() throws java.rmi.RemoteException {
        ArrayList<CtCrisis> result = new ArrayList<>();
        if (cmpSystemCtCrisis != null) {
            result.addAll(cmpSystemCtCrisis.values());
        }
        return result;
    }
    ...
    public ArrayList<ActComCompany> getAllActComCompanies() throws java.rmi.RemoteException {
        ArrayList<ActComCompany> result = new ArrayList<>();
        if (cmpSystemActComCompany != null) {
            result.addAll(cmpSystemActComCompany.values());
        }
        return result;
    }
    ...
    public synchronized PtBoolean oeCloseCrisis(DtCrisisID aDtCrisisID) {
        try {
            ...
            if (currentRequestingAuthenticatedActor instanceof ActCoordinator) {
                ...
                for (CtAlert theAlert : alertkeys) {
