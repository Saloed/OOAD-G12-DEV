...
public abstract class AbstractGUIController implements Initializable {
    ...
    public DatePicker getDatePicker(DtDate defaultDate, int width) {
        ...
        dtpckr.focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
    ...
    public void setUpCoordTables(TableView<CtCoordinator> tblvw, boolean showPassword) {
        TableColumn<CtCoordinator, String> idCol = new TableColumn<>("ID");
        TableColumn<CtCoordinator, String> nameCol = new TableColumn<>("Username");
        TableColumn<CtCoordinator, String> passwordCol = new TableColumn<>("Password");
        idCol.setCellValueFactory(coord -> new ReadOnlyObjectWrapper<>(coord.getValue().id.value.getValue()));
        nameCol.setCellValueFactory(coord -> new ReadOnlyObjectWrapper<>(coord.getValue().login.value.getValue()));
        passwordCol.setCellValueFactory(coord -> new ReadOnlyObjectWrapper<>(coord.getValue().pwd.value.getValue()));
    ...
    public void setUpAdminTables(TableView<CtAdministrator> tblvw, boolean showPassword) {
        TableColumn<CtAdministrator, String> nameCol = new TableColumn<>("Username");
        TableColumn<CtAdministrator, String> passwordCol = new TableColumn<>("Password");
        nameCol.setCellValueFactory(admin -> new ReadOnlyObjectWrapper<>(admin.getValue().login.value.getValue()));
        passwordCol.setCellValueFactory(admin -> new ReadOnlyObjectWrapper<>(admin.getValue().pwd.value.getValue()));
        tblvw.getColumns().add(nameCol);
        if (showPassword)
            tblvw.getColumns().add(passwordCol);
        setColumnsSameWidth(tblvw);
    ...
    public void setUpAlertTables(TableView<CtAlert> tblvw) {
        TableColumn<CtAlert, String> idCol = new TableColumn<>("ID");
        TableColumn<CtAlert, String> dateCol = new TableColumn<>("Date");
        TableColumn<CtAlert, String> timeCol = new TableColumn<>("Time");
        TableColumn<CtAlert, Double> longitudeCol = new TableColumn<>("Longitude");
        TableColumn<CtAlert, Double> latitudeCol = new TableColumn<>("Latitude");
        TableColumn<CtAlert, String> commentCol = new TableColumn<>("Comment");
        TableColumn<CtAlert, String> statusCol = new TableColumn<>("Status");
        idCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().id.value.getValue()));
        dateCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().instant.date.toString()));
        timeCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().instant.time.toString()));
        longitudeCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().location.longitude.value.getValue()));
        latitudeCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().location.latitude.value.getValue()));
        commentCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().comment.value.getValue()));
        statusCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().status.name()));
        tblvw.getColumns().add(idCol);
    ...
    public void setUpCrisesTables(TableView<CtCrisis> tblvw) {
        TableColumn<CtCrisis, String> idCol = new TableColumn<>("ID");
        TableColumn<CtCrisis, String> dateCol = new TableColumn<>("Date");
        TableColumn<CtCrisis, String> timeCol = new TableColumn<>("Time");
        TableColumn<CtCrisis, String> typeCol = new TableColumn<>("Type");
        TableColumn<CtCrisis, Double> longitudeCol = new TableColumn<>("Longitude");
        TableColumn<CtCrisis, Double> latitudeCol = new TableColumn<>("Latitude");
        TableColumn<CtCrisis, String> commentCol = new TableColumn<>("Comment");
        TableColumn<CtCrisis, String> statusCol = new TableColumn<>("Status");
        idCol.setCellValueFactory(alert -> new ReadOnlyObjectWrapper<>(alert.getValue().id.value.getValue()));
        dateCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().instant.date.toString()));
        timeCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().instant.time.toString()));
        longitudeCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().location.longitude.value.getValue()));
        latitudeCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().location.latitude.value.getValue()));
        commentCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().comment.value.getValue()));
        statusCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().status.name()));
        typeCol.setCellValueFactory(crisis -> new ReadOnlyObjectWrapper<>(crisis.getValue().type.name()));
        tblvw.getColumns().add(idCol);
        tblvw.getColumns().add(dateCol);
        tblvw.getColumns().add(timeCol);
    ...
    public void setUpHumansTables(TableView<CtHuman> tblvw) {
        TableColumn<CtHuman, String> idCol = new TableColumn<>("Phone Number");
        TableColumn<CtHuman, String> kindCol = new TableColumn<>("Type");
        idCol.setCellValueFactory(human -> new ReadOnlyObjectWrapper<>(human.getValue().id.value.getValue()));
        kindCol.setCellValueFactory(human -> new ReadOnlyObjectWrapper<>(human.getValue().kind.name()));
        tblvw.getColumns().add(idCol);
    ...
    public void setUpComCompaniesTables(TableView<String> tblvw) {
        ...
        nameCol.setCellValueFactory(comComp -> new ReadOnlyObjectWrapper<>(comComp.getValue()));
        tblvw.getColumns().add(nameCol);
        setColumnsSameWidth(tblvw);
    }
    ...
    public void setUpMessageTables(TableView<Message> tblvw) {
        TableColumn<Message, MessageType> typeCol = new TableColumn<>("Input event");
        TableColumn<Message, String> messageCol = new TableColumn<>("Message");
        typeCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().getMessageType()));
        messageCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().getMessage()));
        tblvw.getColumns().add(typeCol);
    ...
    public void setUpStateTables(TableView<CtState> tblvw) {
        TableColumn<CtState, String> dateCol = new TableColumn<>("Date");
        TableColumn<CtState, String> timeCol = new TableColumn<>("Time");
        TableColumn<CtState, String> crisisReminderCol = new TableColumn<>("Crisis Reminder");
        TableColumn<CtState, String> maxCrisisReminderCol = new TableColumn<>("Max Crisis Reminder");
        TableColumn<CtState, String> lastReminderCol = new TableColumn<>("Last Reminder");
        TableColumn<CtState, Boolean> startedCol = new TableColumn<>("Started");
        dateCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().clock.date.toString()));
        timeCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().clock.time.toString()));
        crisisReminderCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().crisisReminderPeriod.toString()));
        maxCrisisReminderCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().maxCrisisReminderPeriod.toString()));
        lastReminderCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().vpLastReminder.toString()));
        startedCol.setCellValueFactory(param -> new ReadOnlyObjectWrapper<>(param.getValue().vpStarted.getValue()));
