...
public class ICrashCoordGUIController extends AbstractAuthGUIController {
	...
	*/
    ...
    private void reportOnCrisis() {
        CtCrisis crisis = (CtCrisis) getObjectFromTableView(tblvwCrisis);
        if (crisis != null) {
            Dialog<PtBoolean> dialog = new Dialog<>();
    ...
    private void changeCrisisStatus() {
        ...
        if (crisis != null) {
            ...
            txtfldCtCrisisID.setText(crisis.id.value.getValue());
            txtfldCtCrisisID.setDisable(true);
            ComboBox<EtCrisisStatus> cmbbx = new ComboBox<>();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        setUpTables();
        cmbbxAlertStatus.setOnAction(event -> populateAlerts());
        cmbbxCrisisStatus.setOnAction(event -> populateCrisis());
        tbpnMain.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
